...
# the web "Setup script" for rubio-cli-kit's cloud environment.
#
...
if [ -n "$__smoke" ]; then
  echo "cloud-setup: SMOKE ($__smoke) — rubio-cli-kit on $(uname -s)/$(uname -m); cloud-only sections skipped" >&2
else
  echo "cloud-setup: building rubio-cli-kit environment cache (epoch ${CACHE_EPOCH:-unset})" >&2
fi
...
# Python (packaged, root layout): warm the uv-managed virtualenv so the first
# in-session `uv run` is near-instant.
if [ -f pyproject.toml ] && command -v uv >/dev/null 2>&1; then
  uv sync >/dev/null 2>&1 || __warn "uv sync"
elif [ -f pyproject.toml ]; then
  __smoke_check_failed "uv not on PATH after the toolchain bootstrap — the Python warm was skipped"
fi
